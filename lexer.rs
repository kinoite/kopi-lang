        ...
        let kind = match ch {
            ...
            '!' => {
                if let Some((_, '=')) = chars.peek() {
                    ...
                    return Err(("Unexpected character: !".to_string(), start..end));
                }
            }
            '-' => {
                if let Some((_, '-')) = chars.peek() {
            ...
            c if c.is_digit(10) => {
                ...
                match num_str.parse::<f64>() {
                    ...
                    Err(_) => return Err((format!("Invalid number format: '{}'", num_str), start..end)),
                }
            }
            '"' => {
                let mut value = String::new();
                while let Some(&(_, inner_ch)) = chars.peek() {
                    if inner_ch == '"' { break; }
                    value.push(chars.next().unwrap().1);
                }
